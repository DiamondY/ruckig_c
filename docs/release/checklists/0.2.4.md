...
This checklist records the `v0.2.4` patch-release closeout evidence. Local
release gates were rerun from the `0.2.4` release candidate state. Remote push
CI, manual release-random, tag, and GitHub Release evidence are recorded below.

## Scope

- [x] Final commit hash recorded.
- [x] Version is `0.2.4` in `CMakeLists.txt` if a `v0.2.4` tag is being cut.
- [x] Version macros in `include/ruckig_c/ruckig.h` are `0.2.4` if a
  `v0.2.4` tag is being cut.
- [x] `CHANGELOG.md` has a dated `0.2.4` release entry.
- [x] No public C API additions unless separately approved.
- [x] Public header diff reviewed against `v0.2.3`.
...
- [x] Windows performance trend is compared against the `0.2.3` Windows
  release baseline.
- [x] Linux performance trend is compared against the `0.2.3` Linux release
  baseline.
...
- [x] `v0.2.4` annotated tag points at the final release evidence commit.
- [x] GitHub Release `ruckig_c 0.2.4` is published.

## Evidence To Record

```text
Final release commit:
3a65ce47449e4e9fe6708b57c9dc95d7151f2188.
Annotated tag object:
f71e18e95685ebccbe621e37aa115237d70f5bad.
Tag target commit:
3a65ce47449e4e9fe6708b57c9dc95d7151f2188.
Push CI run id:
26961897352.
...
100% tests passed, 0 tests failed out of 1; `ruckig_c_oracle_random_release` passed in 48.88 seconds.
Windows performance result:
average_ratio_c_over_oracle: 1.4087; threshold: 1.5.
Linux performance result:
Manual workflow run `26962279739` completed successfully; Linux Clang performance average ratio: 1.29012; threshold: 1.5.
Windows/Linux performance trend versus 0.2.3:
Windows local release ratio changed from `1.08893` in `0.2.3` to `1.4087` in `0.2.4`; both are below the `1.5` threshold. Linux manual workflow ratio changed from `1.26328` in `0.2.3` to `1.29012` in `0.2.4`; both are below the `1.5` threshold.
...
Push CI run `26961897352` and manual workflow run `26962279739` completed successfully.
Windows clang-cl shared DLL consumer result:
Push CI run `26961897352` and manual workflow run `26962279739` completed successfully.
Linux exported-symbol artifact:
GitHub Actions run `26962279739`, `Linux exported symbols` job artifact.
...
Windows exported-symbol baseline comparison reports 66 current symbols, 66 baseline symbols, 0 added symbols, and 0 removed symbols. Linux and Windows exported-symbol jobs passed in GitHub Actions runs `26961897352` and `26962279739`.
Public header diff review result:
Public header diff from `v0.2.3` contains only version macro changes from `0.2.3` to `0.2.4`; public functions, enum numeric values, and result-code numeric values are unchanged.
