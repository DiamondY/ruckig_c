...
release gates have been rerun from the `0.2.4` release candidate state. Remote
...
Pending push CI or manual release workflow.
Windows/Linux performance trend versus 0.2.3:
Windows local release ratio changed from `1.08893` in `0.2.3` to `1.4087` in `0.2.4`; both are below the `1.5` threshold. Linux release evidence is pending.
...
Windows exported-symbol baseline comparison reports 66 current symbols, 66 baseline symbols, 0 added symbols, and 0 removed symbols.
