...
| Full original repository/product parity | 72-77% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, while formal bindings, package distribution, Pro/cloud equivalence, visualization gallery generation, and C++ ergonomics remain gaps. |
...
the 172-symbol strategy/diagnostics ABI before stable closeout. Original-style
algorithm visualization remains a documentation/evidence gap rather than a
`v0.7.0` blocker: future work should generate `ruckig_c`-owned
trajectory/tracking/waypoint plots locally instead of copying original images
...
The next full-parity step is post-`v0.7.0` visualization evidence and further
