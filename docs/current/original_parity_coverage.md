...
| Bindings and ecosystem | 38-48% | Python and Rust remain prototype/alpha only, now with Fast and Optimized tracking smoke coverage; package-manager recipes are deferred. |
| Full original repository/product parity | 70-75% | Core runtime is strong; Optimized tracking is alpha only, and formal bindings, package distribution, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and `0.6.0-design` Optimized alpha examples exist. | 80-88% | CMake example build and CTest example execution, including Fast and Optimized tracking examples. | Eigen/custom vector examples do not map directly to C; Optimized examples are alpha evidence, not stable Pro parity. | Keep Optimized examples in alpha evidence; only promote to stable release evidence after `v0.6.0` closeout. |
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including Fast and Optimized tracking smoke. | 45-55% | Local 20-test prototype suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, and diagnostics coverage. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after tracking stable-release evidence. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including Fast and Optimized tracking smoke. | 42-52% | Rust alpha smoke, examples, and local 12-test wrapper suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, and diagnostics coverage. | No published crate, no stable Rust API contract, Optimized tracking wrapper is alpha only. | Keep alpha smoke; decide crate scope separately after C tracking stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `0.6.0-design` now adds an alpha bounded local Optimized MVP with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. | 45-55% local tracking scope; 25-35% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; 8 C tracking CTest gates including Optimized, quality, and no-allocation; Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no stable `v0.6.0` release, no package publication, and no formal optimized global optimality proof. | Expand alpha evidence and decide stable `v0.6.0` only after CI, ABI/export, and tracking quality trends are clean. |
...
`v0.5.0` adds stable local Fast tracking evidence, and `0.6.0-design` now adds
alpha local Optimized tracking evidence. The project still must not claim
complete original product parity because Optimized tracking is not yet a stable
release, formal bindings, package publication, cloud/Pro equivalence, and
C++-specific ergonomics remain incomplete or intentionally out of scope.

The next full-parity step is expanding `0.6.0-alpha` Optimized tracking
evidence into a stable-release decision.
