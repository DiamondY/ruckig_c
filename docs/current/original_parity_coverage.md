...
| Bindings and ecosystem | 35-45% | Python and Rust remain prototype/alpha only, now with tracking smoke coverage; package-manager recipes are deferred. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, and tracking alpha examples exist. | 78-86% | CMake example build and CTest example execution, including online Fast tracking and offline sequence examples. | Eigen/custom vector examples do not map directly to C; tracking `Optimized` example is deferred until implemented. | Keep tracking examples in alpha evidence; add Optimized only after behavior exists. |
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including tracking alpha smoke. | 42-52% | Cross-platform Python prototype smoke; local 20-test prototype suite with online/offline, multi-DoF, invalid-parameter, and `Optimized` unsupported tracking coverage. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after tracking alpha evidence stabilizes. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including tracking alpha smoke. | 38-48% | Rust alpha smoke, examples, and local 12-test wrapper suite with online/offline, multi-DoF, invalid-parameter, and `Optimized` unsupported tracking coverage. | No published crate, no stable Rust API contract, `Optimized` tracking unsupported. | Keep alpha smoke; extend after C tracking alpha stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `0.5.0-design` alpha implementation is in progress: public C ABI, online Fast tracking, offline output sequence, C examples, Python smoke, and Rust smoke exist. | 25-35% alpha-local scope; 10-20% versus full Pro surface | `docs/design/tracking_interface.md`; 7 C tracking CTest gates including fixed corpus, tuned quality, and no-allocation; Python/Rust smoke. | `Optimized` mode returns `RUCKIG_ERROR_UNSUPPORTED`; no source-level oracle, no Pro/cloud equivalence, no package publication. | Stabilize local tracking alpha evidence before any stable `v0.5.0` release; implement `Optimized` only after a `0.6.0-design` algorithm decision. |
...
`0.5.0-design` adds tracking alpha evidence, but the project must not claim
complete original product parity because tracking `Optimized`, formal bindings,
package publication, cloud/Pro equivalence, and C++-specific ergonomics remain
incomplete or intentionally out of scope.

The next full-parity step is to stabilize the `0.5.0-design` tracking alpha
evidence before considering a stable `v0.5.0` release.
