...
| Full original repository/product parity | 73-78% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, and `0.8.0-readiness` confirms the local visualization gallery/verifier evidence is ready for stable review. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. `0.7.0-alpha` examples explicitly select strategy presets, and `0.8.0-alpha.2` generates a Matplotlib PNG gallery from public C ABI data for original examples `01-10` and `14-16`; `0.8.0-alpha.3` adds local verifier evidence for those assets; `0.8.0-readiness` reruns focused readiness gates. | 82-89% | CMake example build and CTest example execution, including Fast and Optimized tracking examples; `docs/current/test_coverage_audit.md` maps original examples to current C/Rust/Python evidence; `docs/current/visualization.md` records local generated gallery evidence, example mapping, verifier commands, and readiness audit status. | Eigen/custom vector examples `11-13` do not map directly to C; Optimized examples are local MVP evidence, not stable Pro parity; original-style PDFs are not regenerated and original images are not copied. | Keep gallery generation and verification local-only; decide optional CI artifacts and any further gallery scope separately. |
...
`0.8.0-readiness` records focused local readiness evidence before any stable
closeout. Original images and PDFs remain references only and are not copied as
...
The next full-parity step is deciding whether visualization artifacts should
become optional CI uploads, adding richer diagnostics plots if useful, and
continuing tracking quality/stability deepening.
