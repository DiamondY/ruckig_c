...
| Full original repository/product parity | 74-79% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, `v0.8.0` stabilizes the local visualization gallery/verifier evidence, and `0.9.0-readiness` records full local stable-review evidence for tracking quality/stability without expanding ABI. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. `0.7.0-alpha` examples explicitly select strategy presets, and `v0.8.0` stabilizes a Matplotlib PNG gallery generated from public C ABI data for original examples `01-10` and `14-16`, with local verifier evidence for those assets. | 83-90% | CMake example build and CTest example execution, including Fast and Optimized tracking examples; `docs/current/test_coverage_audit.md` maps original examples to current C/Rust/Python evidence; `docs/current/visualization.md` records local generated gallery evidence, example mapping, verifier commands, and stable release status. | Eigen/custom vector examples `11-13` do not map directly to C; Optimized examples are local MVP evidence, not stable Pro parity; original-style PDFs are not regenerated and original images are not copied. | Keep gallery generation and verification local-only; decide optional CI artifacts and any further gallery scope separately. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `v0.7.0` stabilizes Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress evidence. `0.9.0-alpha` adds deterministic fallback-distribution audit evidence, `0.9.0-alpha.2` tunes the bounded evaluator with private candidate-family attribution and hard 10k/100k/1M thresholds, `0.9.0-alpha.3` freezes representative tuned behavior as stability regression evidence, and `0.9.0-readiness` reruns full local stable-review gates. | 64-74% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; `docs/current/tracking_quality_audit.md`; `docs/current/tracking_quality_hardening.md`; `docs/current/tracking_stability.md`; `docs/release/checklists/0.9.0-readiness.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, quality hardening, stability, random stress, random audit, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. Readiness improves stable-review confidence for the local bounded evaluator but still does not prove global optimality. | If ordinary push CI is green, use `0.9.0-readiness` to decide whether the next separate step can be `v0.9.0` stable closeout without changing the stable ABI by default. |
...
evidence, and `0.9.0-readiness` reruns the full local readiness gate set before
any later stable closeout decision. This modestly improves the local
tracking-scope estimate, but it still does not justify Pro/cloud equivalence or
a formal optimized global optimality claim. Visualization v2, optional CI
uploads, and richer diagnostics plots remain separate scope decisions.
