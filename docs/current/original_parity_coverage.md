...
| Full original repository/product parity | 73-78% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, and `0.8.0-alpha.3` adds verified local visualization gallery evidence. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
equivalents of original examples `01-10` and `14-16`, and `0.8.0-alpha.3`
adds local verifier evidence for the committed PNG/manifest assets. Original
images and PDFs remain references only and are not copied as primary project
evidence.
