...
| Full original repository/product parity | 74-79% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, and `v0.8.0` stabilizes the local visualization gallery/verifier evidence. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `v0.7.0` stabilizes Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress evidence. `0.9.0-alpha` adds deterministic fallback-distribution audit evidence, `0.9.0-alpha.2` tunes the bounded evaluator with private candidate-family attribution and hard 10k/100k/1M thresholds, and `0.9.0-alpha.3` freezes representative tuned behavior as stability regression evidence. | 62-72% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; `docs/current/tracking_quality_audit.md`; `docs/current/tracking_quality_hardening.md`; `docs/current/tracking_stability.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, quality hardening, stability, random stress, random audit, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. Alpha.3 improves regression protection for the local bounded evaluator but still does not prove global optimality. | Keep strategy/diagnostics/stability evidence in stable release checks; use the `0.9.0-alpha.3` stability evidence to decide whether the next step can be readiness without changing the stable ABI by default. |
...
evidence. This modestly improves the local tracking-scope estimate, but it
still does not justify Pro/cloud equivalence or a formal optimized global
optimality claim. Visualization v2, optional CI uploads, and richer diagnostics
plots remain separate scope decisions.
