...
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including tracking alpha smoke. | 42-52% | Cross-platform Python prototype smoke; local 17-test prototype suite with online/offline tracking. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after tracking alpha evidence stabilizes. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including tracking alpha smoke. | 38-48% | Rust alpha smoke, examples, and local 9-test wrapper suite with online/offline tracking. | No published crate, no stable Rust API contract, `Optimized` tracking unsupported. | Keep alpha smoke; extend after C tracking alpha stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `0.5.0-design` alpha implementation is in progress: public C ABI, online Fast tracking, offline output sequence, C examples, Python smoke, and Rust smoke exist. | 25-35% alpha-local scope; 10-20% versus full Pro surface | `docs/design/tracking_interface.md`; C tracking CTest gates; Python/Rust smoke. | `Optimized` mode returns `RUCKIG_ERROR_UNSUPPORTED`; no source-level oracle, no Pro/cloud equivalence, no package publication. | Stabilize local tracking alpha evidence before any stable `v0.5.0` release. |
