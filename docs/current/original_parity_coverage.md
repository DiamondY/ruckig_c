...
| Full original repository/product parity | 73-78% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, and `0.8.0-alpha.2` expands local generated visualization gallery evidence. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
adds local generated visualization gallery evidence, and `0.8.0-alpha.2`
replaces that first gallery with a Matplotlib/NumPy PNG set covering local C ABI
equivalents of original examples `01-10` and `14-16`. Original images and PDFs
remain references only and are not copied as primary project evidence.
