...
| Full original repository/product parity | 72-77% | Core runtime is strong; bounded Optimized tracking is stable and being deepened on `0.7.0-alpha.2`; `0.7.0-alpha.3` adds local coverage audit evidence, `0.7.0-alpha.4` hardens targeted solver branch coverage, and `0.7.0-readiness` records full local stable-review evidence for the 172-symbol ABI candidate while formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. `0.7.0-alpha` examples explicitly select strategy presets. | 80-88% | CMake example build and CTest example execution, including Fast and Optimized tracking examples; `docs/current/test_coverage_audit.md` maps original examples to current C/Rust/Python evidence. | Eigen/custom vector examples do not map directly to C; Optimized examples are local MVP evidence, not stable Pro parity; original-style trajectory PDFs and gallery images are not generated yet. | Keep Optimized examples in release evidence; defer algorithm visualization/gallery generation until after `v0.7.0`. |
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including Fast, Optimized tracking, lookahead, diagnostics snapshots, and `0.7.0-alpha.2` strategy preset smoke. | 45-55% | Local prototype suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, diagnostics snapshot, and strategy preset coverage. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after tracking stable-release evidence. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including Fast, Optimized tracking, lookahead, diagnostics snapshots, and `0.7.0-alpha.2` strategy preset smoke. | 42-52% | Rust alpha smoke, examples, and local wrapper suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, diagnostics snapshot, and strategy preset coverage. | No published crate, no stable Rust API contract, Optimized tracking wrapper is alpha only. | Keep alpha smoke; decide crate scope separately after C tracking stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `0.7.0-alpha.2` deepens Optimized tracking with Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress. `0.7.0-readiness` records full local stable-review evidence for that 172-symbol API candidate. | 60-70% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, random stress, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. | If stable closeout is accepted, move from readiness evidence to `v0.7.0` version/tag/release flow without adding new public API. |
...
`v0.5.0` adds stable local Fast tracking evidence, and `v0.6.0` adds stable
...
the current 172-symbol strategy/diagnostics ABI as a stable-review candidate,
but it intentionally does not bump versions, move ABI artifact paths, create a
tag, or create a GitHub Release. Original-style algorithm visualization remains
a documentation/evidence gap rather than a `v0.7.0` blocker: future work should
generate `ruckig_c`-owned trajectory/tracking/waypoint plots locally instead of
copying original images as primary project evidence.
...
The current full-parity step is `0.7.0-design` tracking quality/stability
deepening.
