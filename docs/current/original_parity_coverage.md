...
| Waypoints, per-section constraints, and global position bounds | Original waypoint examples and input/trajectory surface; implementation source for Pro/cloud behavior is not in the frozen Community tree. | Public C ABI and local coupled waypoint optimizer are implemented; per-section constraints, intermediate durations, extrema, first-time, filtering, and visualization evidence are covered. | 76-82% for current interface/effect scope | `docs/design/0.4.0_original_parity.md`; `0.4.1` waypoint fixed corpus; section-oracle comparisons; waypoint benchmark trend; Visualization v2 waypoint plots. | No formal global optimality proof and limited optional black-box evidence against proprietary implementations. Cloud runtime itself is out of current scope. | Keep local evidence as the routine gate; use optional Pro/cloud samples only as non-blocking effect comparison evidence if available. |
