...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, and tracking Fast-mode examples exist. | 78-86% | CMake example build and CTest example execution, including online Fast tracking and offline sequence examples. | Eigen/custom vector examples do not map directly to C; tracking `Optimized` example is deferred until implemented. | Keep Fast tracking examples in stable evidence; add Optimized only after behavior exists. |
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including tracking smoke. | 42-52% | Cross-platform Python prototype smoke; local 20-test prototype suite with online/offline, multi-DoF, invalid-parameter, and `Optimized` unsupported tracking coverage. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after `v0.5.0` tracking evidence stabilizes. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including tracking smoke. | 38-48% | Rust alpha smoke, examples, and local 12-test wrapper suite with online/offline, multi-DoF, invalid-parameter, and `Optimized` unsupported tracking coverage. | No published crate, no stable Rust API contract, `Optimized` tracking unsupported. | Keep alpha smoke; extend after `v0.5.0` tracking evidence stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes the public C tracking ABI, online Fast tracking, offline output sequence, C examples, Python smoke, and Rust smoke. | 35-45% local Fast scope; 15-25% versus full Pro surface | `docs/design/tracking_interface.md`; 7 C tracking CTest gates including fixed corpus, tuned quality, and no-allocation; Python/Rust smoke. | `Optimized` mode returns `RUCKIG_ERROR_UNSUPPORTED`; no source-level oracle, no Pro/cloud equivalence, no package publication. | Implement `Optimized` only after a `0.6.0-design` algorithm decision. |
...
`v0.5.0` adds stable local Fast tracking evidence, but the project must not claim
...
The next full-parity step is `0.6.0-design` for bounded local `Optimized`
tracking.
