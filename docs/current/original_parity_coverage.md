...
| Bindings and ecosystem | 38-48% | Python and Rust remain prototype/alpha only, now with Fast, Optimized tracking, and `0.7.0-alpha` strategy preset smoke coverage; package-manager recipes are deferred. |
| Full original repository/product parity | 72-77% | Core runtime is strong; bounded Optimized tracking is stable and being deepened on `0.7.0-alpha`, while formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. `0.7.0-alpha` examples explicitly select strategy presets. | 80-88% | CMake example build and CTest example execution, including Fast and Optimized tracking examples. | Eigen/custom vector examples do not map directly to C; Optimized examples are local MVP evidence, not stable Pro parity. | Keep Optimized examples in release evidence and expand tracking quality examples in `0.7.0-design`. |
| Python binding | Original Python examples and package distribution | `bindings/python_prototype` is a `cffi` ABI-mode prototype over the public C ABI, including Fast, Optimized tracking, lookahead, diagnostics, and `0.7.0-alpha` strategy preset smoke. | 45-55% | Local prototype suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, diagnostics, and strategy preset coverage. | No wheel, no stable Python API, no package metadata, no formal package tracking wrapper. | Keep prototype smoke; decide formal package scope after tracking stable-release evidence. |
| Rust binding | Original Rust examples/wrapper | `bindings/rust` is an alpha wrapper over `ruckig_c` handles, including Fast, Optimized tracking, lookahead, diagnostics, and `0.7.0-alpha` strategy preset smoke. | 42-52% | Rust alpha smoke, examples, and local wrapper suite with online/offline, multi-DoF, invalid-parameter, Fast tracking, Optimized tracking, lookahead update, diagnostics, and strategy preset coverage. | No published crate, no stable Rust API contract, Optimized tracking wrapper is alpha only. | Keep alpha smoke; decide crate scope separately after C tracking stabilizes. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `0.7.0-alpha` deepens Optimized tracking with Stable/Balanced/Aggressive strategy presets, deterministic quality gates, and tracking random stress. | 58-68% local tracking scope; 32-42% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; C tracking CTest gates including Optimized, strategy quality, random stress, and no-allocation; Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. | Use `0.7.0-design` alpha evidence to decide whether strategy presets are sufficient for a later stable tracking deepening release. |
...
bounded local Optimized tracking evidence. `0.7.0-alpha` improves local
tracking quality evidence with strategy presets and deterministic stress, but
it is not a stable release and does not create formal Pro/cloud equivalence.
