...
| Full original repository/product parity | 74-79% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, `v0.8.0` stabilizes the first local visualization gallery/verifier evidence, `v0.9.0` stabilizes tracking quality/stability evidence, and `v0.10.0` stabilizes Visualization v2 evidence without expanding ABI. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
