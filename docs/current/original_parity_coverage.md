...
| Full original repository/product parity | 72-77% | Core runtime is strong; bounded Optimized tracking is stable, while formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. | 80-88% | CMake example build and CTest example execution, including Fast and Optimized tracking examples. | Eigen/custom vector examples do not map directly to C; Optimized examples are local MVP evidence, not stable Pro parity. | Keep Optimized examples in release evidence and expand tracking quality examples in `0.7.0-design`. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. | 55-65% local tracking scope; 30-40% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; 8 C tracking CTest gates including Optimized, quality, and no-allocation; Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. | Use `0.7.0-design` to deepen Optimized tracking quality/stability corpus and trend metrics. |
...
bounded local Optimized tracking evidence. The project still must not claim
complete original product parity because formal bindings, package publication,
cloud/Pro equivalence, formal global optimality proof, and C++-specific
ergonomics remain incomplete or intentionally out of scope.

The next full-parity step is `0.7.0-design` tracking quality/stability
deepening.
