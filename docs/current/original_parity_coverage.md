...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `v0.7.0` stabilizes Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress evidence. `0.9.0-alpha` adds deterministic fallback-distribution audit evidence before evaluator tuning. | 60-70% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; `docs/current/tracking_quality_audit.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, random stress, random audit, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. Current audit shows fallback-heavy behavior mainly from candidates not improving Fast score, not candidate solver rejection. | Keep strategy/diagnostics evidence in stable release checks; use the `0.9.0-alpha` audit baseline to guide later evaluator changes without changing the stable ABI by default. |
...
The next full-parity step is tracking quality/stability deepening on the
`0.9.0-design` line. `0.9.0-alpha` starts that work with deterministic
fallback-distribution and diagnostics audit evidence; it does not tune the
Optimized evaluator or raise product-parity estimates yet. Visualization v2,
optional CI uploads, and richer diagnostics plots remain separate scope
decisions.
