...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `v0.7.0` stabilizes Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress evidence. `0.9.0-alpha` adds deterministic fallback-distribution audit evidence before evaluator tuning, and `0.9.0-alpha.2` tunes the bounded evaluator with private candidate-family attribution and hard 10k/100k/1M per-strategy quality thresholds. | 62-72% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; `docs/current/tracking_quality_audit.md`; `docs/current/tracking_quality_hardening.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, quality hardening, random stress, random audit, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. Alpha.2 improves the local bounded evaluator against its alpha.1 audit thresholds but still does not prove global optimality. | Keep strategy/diagnostics evidence in stable release checks; use the `0.9.0-alpha.2` hardening evidence to guide later tracking stability work without changing the stable ABI by default. |
...
This modestly improves the local tracking-scope estimate, but it still does
not justify Pro/cloud equivalence or a formal optimized global optimality
claim. Visualization v2, optional CI uploads, and richer diagnostics plots
