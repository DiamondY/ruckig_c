...
| Full original repository/product parity | 74-79% | Core runtime is strong; `v0.7.0` stabilizes bounded Optimized tracking strategy presets and diagnostics snapshots after alpha coverage/readiness evidence, `v0.8.0` stabilizes the local visualization gallery/verifier evidence, and `v0.9.0` stabilizes tracking quality/stability evidence without expanding ABI. Formal bindings, package distribution, Pro/cloud equivalence, and C++ ergonomics remain gaps. |
...
| C examples | `original/ruckig-main/examples` | Position, offline, online, velocity, stop, minimum duration, per-DoF overrides, waypoints, per-section constraints, filtering, dynamic waypoint, tracking Fast, and Optimized tracking examples exist. `0.7.0-alpha` examples explicitly select strategy presets, `v0.8.0` stabilizes a Matplotlib PNG gallery generated from public C ABI data for original examples `01-10` and `14-16`, and `0.10.0-alpha` replaces the current `main` gallery with 30 local Visualization v2 PNGs covering original mappings, tracking diagnostics, waypoint diagnostics, trajectory anatomy, and summary plots. `0.10.0-readiness` reviews that gallery, verifier, and manual artifact path for stable closeout. | 83-90% | CMake example build and CTest example execution, including Fast and Optimized tracking examples; `docs/current/test_coverage_audit.md` maps original examples to current C/Rust/Python evidence; `docs/current/visualization.md` records local generated gallery evidence, v2 inventory, example mapping, verifier commands, manual-only CI artifact commands, and v1 provenance through the `v0.9.0` tag. | Eigen/custom vector examples `11-13` do not map directly to C; Optimized examples are local bounded evidence, not Pro parity; original-style PDFs are not regenerated and original images are not copied. | Keep committed gallery generation and verification local-first; optional CI artifacts are manual-only review evidence, not a default release gate. |
...
| Tracking interface | README Tracking Interface and examples `14_tracking.*`, `15_tracking_offline.*`; no `trackig.hpp` in frozen Community headers | `v0.5.0` stabilizes Fast tracking. `v0.6.0` stabilizes bounded local Optimized tracking with lookahead update, offline sliding window, candidate budget, fallback diagnostics, C examples, Python smoke, and Rust smoke. `v0.7.0` stabilizes Stable/Balanced/Aggressive strategy presets, diagnostics snapshots, stricter deterministic quality gates, and 100k tracking random stress evidence. `v0.9.0` stabilizes deterministic fallback-distribution audit evidence, bounded evaluator tuning with private attribution and hard 10k/100k/1M thresholds, and fixed stability regression evidence. | 64-74% local tracking scope; 34-44% versus full Pro surface | `docs/design/tracking_interface.md`; `docs/design/tracking_optimized_mode.md`; `docs/current/tracking_quality_audit.md`; `docs/current/tracking_quality_hardening.md`; `docs/current/tracking_stability.md`; `docs/release/checklists/0.9.0.md`; C tracking CTest gates including Optimized, diagnostics, strategy quality, quality hardening, stability, random stress, random audit, no-allocation, 1M release-random readiness, ABI/export checks, and Python/Rust smoke. | No source-level oracle, no Pro/cloud equivalence, no package publication, and no formal optimized global optimality proof. `v0.9.0` improves stable-review confidence for the local bounded evaluator but still does not prove global optimality. | Keep tracking quality/stability gates in release checks; move post-`v0.9.0` default work to visualization v2 unless a separate tracking follow-up is accepted. |
...
a formal optimized global optimality claim. `0.10.0-alpha` starts the next
...
full local stable-review evidence for that Visualization v2 line without
changing public ABI, tracked gallery assets, the frozen original baseline, or
default CI behavior. Stable closeout remains the point where a version bump,
tag, GitHub Release, and manual release-random workflow can happen.
