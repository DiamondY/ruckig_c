...
int main(void) {
    ...
    ruckig_input_max_velocity_data(input)[0] = 3.0;
    ruckig_input_max_acceleration_data(input)[0] = 4.0;
    ...
    for (step = 0; step < sample_count; ++step) {
        ...
        const double w = 0.7;
        position[step] = 0.3 * sin(w * t);
        velocity[step] = 0.3 * w * cos(w * t);
        acceleration[step] = -0.3 * w * w * sin(w * t);
    }
