...
int main(void) {
    ...
    ruckig_input_min_position_data(input)[0] = -2.0;
    ruckig_input_max_position_data(input)[0] = 2.0;
    ...
    for (step = 0; step < 240; ++step) {
        ...
        for (sample = 0; sample < lookahead_count; ++sample) {
            ...
            const double w = 0.8;
            ruckig_target_state_sequence_position_data(targets)[sample] = 0.25 * sin(w * t);
            ruckig_target_state_sequence_velocity_data(targets)[sample] = 0.25 * w * cos(w * t);
            ruckig_target_state_sequence_acceleration_data(targets)[sample] = -0.25 * w * w * sin(w * t);
        }
    ...
    printf(
        "optimized tracking sinus final position: %.6f status: %d candidates: %zu strategy: %d\n",
